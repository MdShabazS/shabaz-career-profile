...
## 2026-08-28 — Tier-1 resume rebuild (Phase 1)

Rebuilt the master resume on the user's proven prior template (which scored ~83), correcting all facts and fixing the writing issues that scored a later draft at 52.

- **Design:** centered navy header, ruled section headings, right-aligned dates — matching the reference. Header now shows clickable **LinkedIn** and **GitHub** labels with icons (no raw URLs); email shown and clickable. Icons generated by `scripts/make_icons.py` into `resume/assets/`.
- **Nokia** stated plainly ("Working as a Student Intern at Nokia.") — confidentiality wording removed per instruction; dates 16 Sep 2026 – Present.
- **Bullets:** every bullet leads with a distinct verb (fixing verb repetition), and honest quantifiers surfaced (2 video sources, 3 mechanisms, 5-minute fix, 6+ improvements, ~400 images/6 classes, 4 mechanisms, ~93%, 3-state/6+ functions, 2 sensor types, 3-member, 15-stage) — no fabricated metrics.
- **Content:** 4 projects (VisionPay, Automotive BCM, Smart Wellness [team], AEGIS [in progress]); MITRA under iHelp; Skin Disease under IEEE EMBS. Locked skills (Microcontrollers; no boards/REST/RTOS as skills); specific platforms only in bullets.
- **Tooling:** new `scripts/validate_resume.py` (parse/format/leakage checks + readiness score + plain-text export) replaces `scripts/ats_check.py`; new `research/TIER1_RESUME_RESEARCH.md` (resume research moved out of the role-research doc); `ats-analysis.md` expanded with per-role JD keyword tables.
- Generated `resume/Mohammed_Shabaz_S_Master_Resume.{pdf,docx,txt}`. Readiness ~96/100 (breakdown shown); repo and resume validators pass.
- Commit: `feat: build tier-1 ats optimized master resume`.
